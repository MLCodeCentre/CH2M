...
% fist lane marker
x = 1.6253; y = 5.7749 - 2.05; z = -h; u = 2111; v = 1983;

eq1 = (u-cx) - m*L1*((R(1,1)*x + R(1,2)*y + R(1,3)*z)...
...
x = 1.2111; y = 28.3362 - 2.05; z = -h; u = 1411; v = 990;

eq3 = (u-cx) - m*L1*((R(1,1)*x + R(1,2)*y + R(1,3)*z)...
                    /(R(2,1)*x + R(2,2)*y + R(2,3)*z));
...
% vanishing points
up = 1290; 
eq5 = (up-cx) - m*L1*((-sin(G)*cos(A)+cos(G)*sin(B)*sin(A))...
                       /(cos(B)*cos(A)+sin(G)*sin(B)*sin(A)));
                   
vp = 803; 
eq6 = (cy-vp) - n*L2*((cos(B)*sin(A))...
                     /(cos(B)*cos(A)+sin(G)*sin(B)*sin(A)));
