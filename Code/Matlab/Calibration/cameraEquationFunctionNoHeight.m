function F = cameraEquationFunctionNoHeight(theta,coords,system_params)

% constants
A = theta(1); B = theta(2); G = theta(3); L1 = theta(4); L2 = theta(5); h = theta(6);
x0 = system_params(5); y0 = system_params(6);

x = coords(:,1); y = coords(:,2); u = coords(:,3); v = coords(:,4);
...
for i = 1:num_data_points
    ...
    F(2*i-1)  = ((u(i)*R(1,1) - m*L1*R(2,1))*(x(i)-x0) + (u(i)*R(1,2) - m*L1*R(2,2))*(y(i)-y0) + (u(i)*R(1,3) - m*L1*R(2,3))*(z-h));
    F(2*i)= ((v(i)*R(1,1) - n*L2*R(3,1))*(x(i)-x0) + (v(i)*R(1,2) - n*L2*R(3,2))*(y(i)-y0) + (v(i)*R(1,3) - n*L2*R(3,3))*(z-h));
end
